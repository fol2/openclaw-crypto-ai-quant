...
the daemon `status_path` that later service supervision can watch, including
the effective config selected by
`AI_QUANT_PROMOTED_ROLE` and `AI_QUANT_STRATEGY_MODE` /
`AI_QUANT_STRATEGY_MODE_FILE`. Python paper execution remains the active
