<script lang="ts">
  ...
  import {
    fmtNum, fmtAge, sigAge, pnlPct, pnlClass,
    entryActionLabel, journeySourceLabel, tradeTimeLabel,
    intervalToMs, fmtDuration, pickJourneyInterval, journeyTimeRange,
  } from '../lib/format';
  ...
  function sameContext(requestMode: string, requestSymbol: string): boolean {
  ...
  function sameJourneyContext(
    ...
    return seq === journeyFetchSeq
      && sameContext(requestMode, requestSymbol)
  ...
  async function reconcileCandlesForCurrentView(sym = symbol, iv = selectedInterval, bars = selectedBars): Promise<void> {
    ...
    try {
      const res = await getCandles(sym, iv, bars, mode);
      ...
      if (candles.length > 0) {
        ...
        if (nT > 0 && nT < currentBarStart) {
          ...
          const mid = finitePositive(liveMid) ?? prevClose;
  ...
  async function fetchTunnelForLive() {
    ...
    try {
      ...
      if (!sameContext(requestMode, requestSymbol) || !position) return;
  ...
  async function fetchJourneys(reset = false) {
    ...
    try {
      ...
      if (requestSeq !== journeyListSeq || !sameContext(requestMode, requestSymbol)) return;
  ...
  async function selectJourney(j: any) {
    ...
    try {
      const res = await getCandlesRange(j.symbol, iv, tr.fromTs, tr.toTs, 500, mode);
  ...
  async function changeJourneyInterval(newIv: string) {
    ...
    const seq = ++journeyFetchSeq;
    try {
      const res = await getCandlesRange(j.symbol, newIv, tr.fromTs, tr.toTs, 500, mode);
      if (!sameJourneyContext(seq, requestMode, requestSymbol, requestJourneyId)) return;
      journeyCandles = res.candles || [];
    } catch {}
  }
  ...
  async function onJourneyNeedCandles(e: CustomEvent) {
    ...
    const seq = ++journeyFetchSeq;
    try {
      const res = await getCandlesRange(j.symbol, journeyInterval, newFrom, newTo, 2000, mode);
      if (!sameJourneyContext(seq, requestMode, requestSymbol, requestJourneyId)) return;
  ...
  async function onMainNeedCandles(e: CustomEvent) {
    ...
    mainExtending = true;
    try {
      const res = await getCandlesRange(sym, iv, undefined, before, 500, mode);
  ...
  // ── Lifecycle: Fetch marks + candles on symbol/mode change ───────────
  let _prevSymbol = '';
  let _prevInterval = '';
  let _prevMode = '';
  $effect(() => {
  ...
  // ── Reactive tunnel fetch: re-fetch when position changes ────────────
  $effect(() => {
    const pos = position;
    const sym = symbol;
    if (!pos || !sym) { tunnelPoints = []; return; }
    void fetchTunnelForLive();
  });
...
<div class="panel-root" class:is-dragging={chartDragging || journeyChartDragging}>
  <!-- Tab bar -->
  ...
  {#if detailTab === 'detail'}
    ...
    {#if isLiveMode && manualTradeEnabled}
      <trade-panel
        symbol={symbol}
        ...
        ontradedone={async () => { if (onTradeRefresh) await onTradeRefresh(); try { marks = await getMarks(symbol, mode); } catch {} }}
      ></trade-panel>
    {/if}
...
<style>
  ...
  .journey-leg-row:last-child { border-bottom: none; }
  .journey-leg-top {
    display: flex;
    align-items: center;
    gap: 6px;
