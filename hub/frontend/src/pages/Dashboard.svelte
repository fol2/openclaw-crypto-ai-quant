  ...
  <div class="panel detail-panel" class:mobile-visible={mobileTab === 'detail' || mobileTab === 'feed'}>
    ...
    {#if focusSym}
      <SymbolDetailPanel
        symbol={focusSym}
        mode={currentMode()}
        symbolData={snap?.symbols?.find((s: any) => s.symbol === focusSym)}
        liveMid={snap?.symbols?.find((s: any) => s.symbol === focusSym)?.mid ?? 0}
        recent={snap?.recent || {}}
        isLiveMode={selectedModeKey === LIVE_MODE}
        onTradeRefresh={async () => { await refresh(); }}
      />
    {:else}
