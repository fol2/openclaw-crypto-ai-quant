...
### H3. Database Connection Resource Leak — DEFERRED
...
**Reason deferred:** Code review confirmed existing `try/finally conn.close()` pattern is consistent and adequate across both files. The "stale cursor" scenario in trader.py is handled by the outer `finally` block at line 516-520 which always closes the connection. Refactoring to `with` context managers would touch many methods in OMS and change transactional semantics (auto-commit on exit). Risk of regression outweighs the marginal improvement. No action needed.
