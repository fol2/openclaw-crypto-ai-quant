...
#[test]
fn cpu_gpu_parity_respects_modular_entry_sizing_profile() {
    if let Err(e) = CudaDevice::new(0) {
        eprintln!("Skipping: CUDA unavailable: {:?}", e);
        return;
    }
    ...
    cfg.trade.enable_ssf_filter = false;
    cfg.trade.enable_reef_filter = false;
    cfg.trade.enable_dynamic_sizing = true;
    cfg.trade.confidence_mult_high = 1.8;
    cfg.trade.confidence_mult_medium = 1.4;
    cfg.trade.confidence_mult_low = 1.2;
    cfg.trade.adx_sizing_min_mult = 0.55;
    cfg.trade.adx_sizing_full_adx = 18.0;
    cfg.trade.vol_baseline_pct = 0.01;
    cfg.trade.vol_scalar_min = 0.6;
    cfg.trade.vol_scalar_max = 1.7;
    cfg.trade.bump_to_min_notional = true;
    cfg.trade.leverage = 1.0;
    cfg.trade.allocation_pct = 0.10;
    cfg.trade.slippage_bps = 0.0;
    cfg.trade.enable_pyramiding = false;
    ...
    cfg.runtime = RuntimeConfig {
        profile: "gpu_modular".to_string(),
        ..RuntimeConfig::default()
    };
    cfg.pipeline = PipelineConfig {
        default_profile: "production".to_string(),
        profiles: BTreeMap::from([(
            "gpu_modular".to_string(),
    ...
    assert!(
        balance_rel_err <= 0.05,
        "modular profile balance drift too large (cpu={:.2}, gpu={:.2}, rel_err={:.4})",
        cpu_rpt.final_balance,
        gpu_rpt.final_balance,
        balance_rel_err
    );
    ...
    assert!(
        pnl_rel_err <= 0.05,
        "modular profile pnl drift too large (cpu={:.2}, gpu={:.2}, rel_err={:.4})",
        cpu_rpt.total_pnl,
        gpu_rpt.total_pnl,
        pnl_rel_err
    );
...
#[test]
fn cpu_gpu_parity_respects_signal_mode_reorder_profile() {
    if let Err(e) = CudaDevice::new(0) {
        eprintln!("Skipping: CUDA unavailable: {:?}", e);
        return;
    }
    ...
    let mut bars_1h = Vec::new();
    for i in 0..220 {
        let close = 100.0 + i as f64 * 0.7 + (i as f64 * 0.15).sin() * 2.0;
        bars_1h.push(bar_1h(i as i64 * hour, close));
    }
    let mut candles: CandleData = FxHashMap::default();
    candles.insert("BTC".to_string(), bars_1h);
    ...
    assert!(
        balance_rel_err <= 0.05,
        "signal mode reorder balance drift too large (cpu={:.2}, gpu={:.2}, rel_err={:.4})",
        cpu_rpt.final_balance,
        gpu_rpt.final_balance,
        balance_rel_err
    );
    ...
    assert!(
        pnl_rel_err <= 0.05,
        "signal mode reorder pnl drift too large (cpu={:.2}, gpu={:.2}, rel_err={:.4})",
        cpu_rpt.total_pnl,
        gpu_rpt.total_pnl,
        pnl_rel_err
    );
...
#[test]
fn cpu_gpu_parity_respects_full_tp_ahead_of_partial_profile() {
    if let Err(e) = CudaDevice::new(0) {
        eprintln!("Skipping: CUDA unavailable: {:?}", e);
        return;
    }
    ...
    let profile = "gpu_exit_tp_full_first";
    let mut cfg = StrategyConfig::default();
    cfg.filters.enable_ranging_filter = false;
    cfg.filters.enable_anomaly_filter = false;
    cfg.filters.enable_extension_filter = false;
    ...
    assert!(
        balance_rel_err <= 0.05,
        "full-before-partial exit order balance drift too large (cpu={:.2}, gpu={:.2}, rel_err={:.4})",
        cpu_rpt.final_balance,
        gpu_rpt.final_balance,
        balance_rel_err
    );
    ...
    assert!(
        pnl_rel_err <= 0.05,
        "full-before-partial exit order pnl drift too large (cpu={:.2}, gpu={:.2}, rel_err={:.4})",
        cpu_rpt.total_pnl,
        gpu_rpt.total_pnl,
        pnl_rel_err
    );
