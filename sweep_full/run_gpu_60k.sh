...
# GPU parameter sweep: 17 phases × 4 intervals = ~60K combos
# Uses GPU for ALL phases including indicator-heavy ones (p08/p09/p14)
    ...
    with open(fpath) as f:
        for line in f:
            ...
            row = {
                ...
                "win_rate": round(r.get("win_rate", 0) * 100, 2) if r.get("win_rate", 0) <= 1.0 else round(r.get("win_rate", 0), 2),
                "profit_factor": r.get("profit_factor", 0),
                "max_drawdown_pct": round(r.get("max_drawdown_pct", 0) * 100, 2) if r.get("max_drawdown_pct", 0) <= 1.0 else round(r.get("max_drawdown_pct", 0), 2),
            }
...
csv_path = os.path.join(os.path.dirname(outdir), "sweep_gpu_60k_results.csv")
