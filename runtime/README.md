...
| `paper manifest` | Resolve the current Rust paper daemon service contract | Read-only env/CLI manifest, including launch readiness and restart/resume state |
...
- a read-only service manifest so operators can inspect the resolved Rust daemon launch contract, bootstrap requirement, and restart/resume state before cutover
