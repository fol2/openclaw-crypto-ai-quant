...
| `paper loop` | Execute a bounded Rust paper catch-up loop | Resumes from `runtime_cycle_steps`, optional follow polling, and consumes any `--symbols-file` once at startup |
| `paper daemon` | Execute an opt-in long-running Rust paper orchestration wrapper | Owns watched `--symbols-file` reloads, can wait on an empty watchlist file, and keeps the same `paper cycle` write contract; not cutover |
...
- daemon-owned `--watch-symbols-file` refresh between follow-mode scheduling inspections
