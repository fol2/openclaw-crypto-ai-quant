...
| `paper daemon` | Execute an opt-in long-running Rust paper daemon surface | Owns the outer scheduler, can wait on an empty watchlist file, retains the last good manifest on bad reloads, and keeps the same `paper cycle` write contract; not cutover |
...
scheduler while still reusing the existing follow-mode loop / cycle contracts,
and it already establishes the contracts that later slices will build on:
...
- an opt-in daemon surface that keeps the same follow-mode loop / cycle contracts alive without claiming service cutover
- daemon-owned manifest reporting and last-good retention for refreshed `--symbols-file` lanes
