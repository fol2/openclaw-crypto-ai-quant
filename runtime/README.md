...
| `paper manifest` | Resolve the current Rust paper daemon service contract | Read-only env/CLI manifest, not a daemon |
...
- a read-only service manifest so operators can inspect the resolved Rust daemon launch contract before cutover
