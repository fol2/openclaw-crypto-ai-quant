...
fn write_effective_config(
    ...
    let document = preserve_factory_root_document(base_document, base_cfg)?;
    let header = format!(
        "# Generated by aiq-factory at {}\n# Source: {}\n",
        Utc::now().to_rfc3339_opts(SecondsFormat::Secs, true),
        base_config_path.display()
    );
    let yaml = serde_yaml::to_string(&document).context("serialise effective factory config")?;
    fs::write(&path, format!("{header}{yaml}"))
        .with_context(|| format!("write effective config {}", path.display()))?;
    Ok(path)
}

fn preserve_factory_root_document(
    base_document: &serde_yaml::Value,
    global_cfg: &StrategyConfig,
) -> Result<serde_yaml::Value> {
    let mut root = base_document
        .as_mapping()
        .cloned()
        .ok_or_else(|| anyhow!("factory config document must have a mapping root"))?;
    root.insert(
...
fn role_strategy_mode(role: &str) -> Option<&str> {
...
fn write_document_with_header(
    ...
    let body = serde_yaml::to_string(document)
        .with_context(|| format!("serialise YAML document {}", path.display()))?;
...
fn evaluate_config_performance(
    ...
    write_document_with_header(&runtime_config_path, &header, &runtime_document)?;
...
fn generate_candidate_config(
    ...
    let document = preserve_factory_root_document(base_document, &cfg)?;
    let header = format!(
    ...
    let body = serde_yaml::to_string(&document).context("serialise candidate config YAML")?;
...
fn current_target_config_id(path: &Path, role: &str) -> Result<Option<String>> {
    ...
    let raw_document = read_yaml_document(path)?;
    let cfg = load_config_document_checked(&raw_document, None, false, role_strategy_mode(role))
        .map_err(|err| anyhow!(err))
...
mod tests {
    ...
    fn config_with_lane_modes_yaml() -> &'static str {
        r#"
global:
  engine:
    interval: 30m
...
"#
    ...
    fn strategy_fingerprint_for_role(yaml: &str, role: &str) -> String {
        let raw_document: serde_yaml::Value = serde_yaml::from_str(yaml).unwrap();
        let cfg =
            load_config_document_checked(&raw_document, None, false, role_strategy_mode(role))
                .map_err(|err| anyhow!(err))
                .unwrap();
        strategy_config_fingerprint_sha256(&cfg)
    }
    ...
    #[test]
    fn generated_candidate_config_keeps_fallback_lane_overlay() {
        let dir = tempfile::tempdir().unwrap();
        let output_path = dir.path().join("candidate.yaml");
        let raw_document: serde_yaml::Value =
            serde_yaml::from_str(config_with_lane_modes_yaml()).unwrap();
        let base_cfg = load_config_document_checked(&raw_document, None, false, None)
            .map_err(|err| anyhow!(err))
            .unwrap();
        let sweep = SweepCandidateRow {
            candidate_mode: true,
            config_id: "row".to_string(),
            max_drawdown_pct: 0.1,
            overrides: BTreeMap::from([("trade.allocation_pct".to_string(), 0.07)]),
            profit_factor: 1.3,
            total_pnl: 20.0,
            total_trades: 40,
        };
        ...
        let candidate_document = read_yaml_document(&output_path).unwrap();
        assert!(candidate_document.get("modes").is_some());
        assert!(candidate_document.get("symbols").is_some());
        let fallback_cfg =
            load_config_document_checked(&candidate_document, None, false, Some("fallback"))
                .map_err(|err| anyhow!(err))
                .unwrap();
        assert_eq!(fallback_cfg.engine.interval, "1h");
        assert_eq!(fallback_cfg.engine.entry_interval, "15m");
        assert_eq!(
            strategy_config_fingerprint_sha256(&fallback_cfg),
            strategy_fingerprint_for_role(&fs::read_to_string(&output_path).unwrap(), "fallback"),
        );
    }

    #[test]
    fn generated_candidate_config_keeps_conservative_lane_overlay() {
        let dir = tempfile::tempdir().unwrap();
        let output_path = dir.path().join("candidate.yaml");
        let raw_document: serde_yaml::Value =
            serde_yaml::from_str(config_with_lane_modes_yaml()).unwrap();
        ...
        let sweep = SweepCandidateRow {
            ...
            overrides: BTreeMap::from([("trade.allocation_pct".to_string(), 0.04)]),
            profit_factor: 1.1,
            total_pnl: 10.0,
            total_trades: 24,
        };
        ...
        generate_candidate_config(
            ...
            &sweep.overrides,
            &output_path,
            "candidate_conservative_rank1",
            ShortlistMode::Conservative,
            1,
            &sweep,
        )
        .unwrap();
        ...
        let candidate_document = read_yaml_document(&output_path).unwrap();
        let conservative_cfg =
            load_config_document_checked(&candidate_document, None, false, Some("conservative"))
                .map_err(|err| anyhow!(err))
                .unwrap();
        assert_eq!(conservative_cfg.engine.interval, "4h");
        assert_eq!(conservative_cfg.engine.entry_interval, "1h");
    }

    #[test]
    fn current_target_config_id_uses_role_strategy_mode() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("paper2.yaml");
        fs::write(&path, config_with_lane_modes_yaml()).unwrap();

        let fallback_id = current_target_config_id(&path, "fallback")
            .unwrap()
            .unwrap();

        assert_eq!(
            fallback_id,
            strategy_fingerprint_for_role(config_with_lane_modes_yaml(), "fallback")
        );
        assert_ne!(
            fallback_id,
            strategy_fingerprint_from_yaml(config_with_lane_modes_yaml())
        );
    }
