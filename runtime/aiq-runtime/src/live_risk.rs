...
mod tests {
    ...
    #[test]
    fn refresh_rebuilds_runtime_buckets_when_env_limits_change() {
        let _guard = env_lock().lock().unwrap_or_else(|err| err.into_inner());
        let day_one_ms = utc_ms(2025, 3, 8);
        let _env = EnvGuard::set(&[
            ("AI_QUANT_RISK_MAX_ENTRY_ORDERS_PER_MIN", Some("1")),
        ...
        let _env_reload = EnvGuard::set(&[
            ("AI_QUANT_RISK_MAX_ENTRY_ORDERS_PER_MIN", Some("3")),
            ("AI_QUANT_RISK_MAX_EXIT_ORDERS_PER_MIN", Some("3")),
            ("AI_QUANT_RISK_MAX_CANCELS_PER_MIN", Some("3")),
        ]);
        risk.refresh(day_one_ms + 2_000, Some(1_000.0));

        assert!(
            risk.allow_order(OrderCheck {
                now_ms: day_one_ms + 2_000,
                symbol: "BTC",
                action: OrderAction::Open,
                reduce_risk: false,
            })
