...
use crate::live_risk::{LiveRiskManager, OrderAction, OrderCheck, OrderRecord};
...
fn submit_cycle_plans(
    ...
    for plan in plans {
        submit_plan(client, oms, risk, plan, step_close_ts_ms)?;
    }
    Ok(())
}
...
fn ingest_recent_fills(
    ...
    for fill in fills {
        if let Some(input) = parse_fill_match_input(&fill.raw)? {
            ...
            if inserted && input.exchange_order_id.is_none() {
                warnings.push(format!(
