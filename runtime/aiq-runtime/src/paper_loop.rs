...
pub(crate) struct WorkingPaperDb {
    path: PathBuf,
    cleanup: bool,
}
...
pub fn run_loop(input: PaperLoopInput<'_>) -> Result<PaperLoopReport> {
    ...
    let resolved_symbols = resolve_symbols_for_inspection(&explicit_symbols, input.symbols_file)?;
...
impl PaperLoopStepReport {
    pub(crate) fn from_cycle_report(value: paper_cycle::PaperCycleReport, dry_run: bool) -> Self {
        Self {
...
pub(crate) fn resolve_symbols_for_inspection(
    explicit_symbols: &[String],
...
pub(crate) fn prepare_working_paper_db(paper_db: &Path, dry_run: bool) -> Result<WorkingPaperDb> {
    if !dry_run {
...
mod tests {
    ...
    #[test]
    fn loop_reads_symbols_file_once_at_startup() {
        let dir = tempdir().unwrap();
        let paper_db = dir.path().join("paper.db");
        let candles_db = dir.path().join("candles.db");
        ...
        assert_eq!(report.explicit_symbols, vec!["ETH".to_string()]);
    }
