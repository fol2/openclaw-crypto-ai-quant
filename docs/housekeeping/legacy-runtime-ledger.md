...
|---|---|---|---|
| Python daemon orchestration | `engine/daemon.py`, `engine/core.py` | Recovery-only, non-authoritative. `paper` is legacy recovery/debug only; `live` / `dry_live` now require an explicit archival override and are no longer the production service path | `aiq-runtime paper daemon`, `aiq-runtime paper manifest`, `aiq-runtime paper status`, `aiq-runtime paper service`, `aiq-runtime paper service apply`, `aiq-runtime live manifest`, `aiq-runtime live status`, `aiq-runtime live service`, `aiq-runtime live service apply`, `aiq-runtime live daemon`, `scripts/run_paper_lane.sh`, `scripts/run_live.sh` |
...
| Python live execution | `live/trader.py`, `exchange/executor.py` | Archival recovery/debug only; no longer the authoritative production path | Rust live adapter |
| Python OMS / risk runtime | `engine/oms.py`, `engine/risk.py` | Archival recovery/debug only; no longer the authoritative production path | Rust persistence + risk layers |
