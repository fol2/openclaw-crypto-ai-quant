...
|---|---|---|---|
| Python daemon orchestration | `engine/daemon.py`, `engine/core.py` | Active, frozen (paired opt-in Rust paper daemon owns the outer scheduler/watchlist reload path, Rust now also owns the conventional `paper1/paper2/paper3/livepaper` lane presets plus repo launch examples, and `paper service apply` can supervise the Rust daemon; Python still owns active production cutover, watchdog integration, and the currently deployed service entrypoints) | `aiq-runtime paper daemon`, `aiq-runtime paper manifest`, `aiq-runtime paper status`, `aiq-runtime paper service`, `aiq-runtime paper service apply`, `scripts/run_paper_lane.sh` |
