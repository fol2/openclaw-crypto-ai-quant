...
| Python effective-config/default mirror | `strategy/mei_alpha_v1.py`, `engine/strategy_manager.py`, `engine/promoted_config.py` | Active, frozen compatibility shim (`StrategyManager` still hot-reloads the resolver-selected YAML path, but Rust now owns effective-config resolution) | Rust effective-config resolver (`aiq-runtime paper effective-config`, `paper manifest`, and the shared runtime config contract) |
