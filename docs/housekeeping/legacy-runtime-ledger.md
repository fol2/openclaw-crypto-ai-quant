...
|---|---|---|---|
| Python daemon orchestration | `engine/daemon.py`, `engine/core.py` | Active, frozen (paired opt-in Rust paper daemon owns the outer scheduler/watchlist reload path, while Rust now also owns opt-in Rust daemon supervision via `paper service apply`; Python still owns active production cutover, watchdog integration, and the default service entrypoints) | `aiq-runtime paper daemon`, `aiq-runtime paper manifest`, `aiq-runtime paper status`, `aiq-runtime paper service`, `aiq-runtime paper service apply` |
