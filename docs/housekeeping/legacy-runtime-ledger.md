...
|---|---|---|---|
| Python daemon orchestration | `engine/daemon.py`, `engine/core.py` | Active, frozen (paired opt-in Rust paper daemon owns the outer scheduler/watchlist reload path, Rust now also owns opt-in daemon supervision via `paper service apply`, and the conventional `paper1` / `paper2` / `paper3` / `livepaper` launch mapping now lives in the Rust `paper lane` wrapper; Python still owns active production cutover, watchdog integration, and the default systemd entrypoints) | `aiq-runtime paper daemon`, `aiq-runtime paper manifest`, `aiq-runtime paper status`, `aiq-runtime paper service`, `aiq-runtime paper service apply`, `aiq-runtime paper lane …` |
