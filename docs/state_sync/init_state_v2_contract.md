...
- when `paper loop` is given a `--symbols-file`, it loads that file once at start-up and keeps a bounded manifest for the life of the shell.
- in follow mode, an initially empty `--symbols-file` is still a fail-closed configuration for the bounded loop surface unless open paper positions keep the lane active.
...
- when `paper daemon` is given a `--symbols-file`, `--watch-symbols-file` makes the daemon own later manifest refreshes without restarting; explicit `--symbols` remain additive and open paper positions are still unioned at execution time.
