...
The core Python daemon now owns `dry_live` and `live` from a single entrypoint.
`paper` is a guarded recovery-only compatibility path after the Rust paper
cutover.
...
- **aiq-runtime** (`runtime/aiq-runtime/`): Rust runtime CLI. It now owns the active production paper daemon together with pipeline/bootstrap planning, snapshot export/validate/seed, paper bootstrap/restore diagnostics via `paper doctor`, single-symbol execution via `paper run-once`, a repeatable multi-symbol `paper cycle` shell with explicit step identity, and a bounded `paper loop` catch-up shell that can optionally keep polling after catch-up instead of exiting idle. Rust also owns read-only `paper effective-config`, `paper manifest`, `paper status`, and `paper service` control-plane surfaces for the active paper daemon, plus dedicated live-facing `live effective-config` and `live manifest` surfaces that resolve the current live launch contract, safety-gate state, and config identity while live execution remains Python-owned. The shared runtime materialisation step means `config_path` is the execution-facing YAML consumed by factory wrappers and any guarded Python recovery path, and `paper cycle` reads the resolved runtime stage graph and configured ranker directly, so `production`, `parity_baseline`, `stage_debug`, and custom pipeline profiles can switch between persistent execution and preview-only parity workflows without code forks. The split `paper status` / `paper service` / `paper service apply` surfaces separate read-only planning from the mutating supervisor, while `paper daemon --watch-symbols-file` can reload a configured symbols file, keep the last good manifest on invalid reloads, continue to union open positions into the active symbol set, start cleanly even before a lane-default watched symbols file exists, and persist a daemon lifecycle status JSON at the resolved `status_path`. First launch may be unlocked either by an explicit `AI_QUANT_PAPER_START_STEP_CLOSE_TS_MS` or by `AI_QUANT_PAPER_BOOTSTRAP_FROM_LATEST_COMMON_CLOSE=1`; once `runtime_cycle_steps` exist, that bootstrap value is ignored on later restarts.
