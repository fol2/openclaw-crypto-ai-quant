...
The Rust runtime now owns the authoritative paper and live service paths.
The remaining Python daemon surfaces are archival recovery/debug compatibility
paths only.
...
- **Mei Alpha v1** (`strategy/mei_alpha_v1.py`): Legacy Python strategy and paper execution surface. After the Rust paper cutover it remains a recovery-only fallback, not the authoritative paper runtime.
...
- **LiveTrader** (`live/trader.py`): Legacy Python live execution wrapper retained only for archival recovery/debug workflows.
- **aiq-runtime** (`runtime/aiq-runtime/`): Rust runtime CLI. It now owns the active production paper daemon together with pipeline/bootstrap planning, snapshot export/validate/seed, paper bootstrap/restore diagnostics via `paper doctor`, single-symbol execution via `paper run-once`, a repeatable multi-symbol `paper cycle` shell with explicit step identity, and a bounded `paper loop` catch-up shell that can optionally keep polling after catch-up instead of exiting idle. Rust also owns live-facing `live effective-config`, `live manifest`, `live status`, `live service`, `live service apply`, and `live daemon` surfaces for the production live lane. The Rust live daemon now owns exchange-backed state sync, strategy-state build, pipeline execution, OMS intent/order/fill transitions, risk enforcement, broker submission, fill backfill, and lifecycle status publication. The shared runtime materialisation step means `config_path` is the execution-facing YAML consumed by factory wrappers and any guarded Python recovery path, and the Rust runtime reads the resolved runtime stage graph and configured ranker directly, so `production`, `parity_baseline`, `stage_debug`, and custom pipeline profiles can switch between persistent execution and preview-only parity workflows without code forks. The split `paper status` / `paper service` / `paper service apply` and matching `live status` / `live service` / `live service apply` surfaces separate read-only planning from the mutating supervisor. First paper launch may be unlocked either by an explicit `AI_QUANT_PAPER_START_STEP_CLOSE_TS_MS` or by `AI_QUANT_PAPER_BOOTSTRAP_FROM_LATEST_COMMON_CLOSE=1`; once `runtime_cycle_steps` exist, that bootstrap value is ignored on later restarts. Live cutover continues to respect the staged ramp and rollback safeguards documented in the runbook.
