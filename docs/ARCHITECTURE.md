...
- **aiq-runtime** (`runtime/aiq-runtime/`): Rust runtime CLI. Currently owns pipeline/bootstrap planning, snapshot export/validate/seed, paper bootstrap/restore diagnostics via `paper doctor`, single-symbol execution via `paper run-once`, and a repeatable multi-symbol `paper cycle` shell with explicit step identity.
