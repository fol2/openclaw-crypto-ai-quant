...
- **aiq-runtime** (`runtime/aiq-runtime/`): Rust runtime CLI. Currently owns pipeline/bootstrap planning, snapshot export/validate/seed, paper bootstrap/restore diagnostics via `paper doctor`, single-symbol execution via `paper run-once`, a repeatable multi-symbol `paper cycle` shell with explicit step identity, a bounded `paper loop` catch-up shell that can optionally keep polling after catch-up instead of exiting idle, and a paired opt-in `paper daemon` wrapper that owns the outer scheduler for the same paper cycle contract without claiming paper daemon cutover. Rust now also owns a read-only `paper effective-config` surface for shared control-plane consumers, a Rust-owned runtime materialisation step whose `config_path` is the execution-facing YAML consumed by Python paper/factory wrappers, a read-only `paper manifest` surface that resolves the current daemon service/env contract into a deterministic launch plan with launch readiness plus restart/resume state, and conventional lane presets for `paper1`, `paper2`, `paper3`, and `livepaper`. Python paper start-up and factory materialisation now consume the same promoted-role / strategy-mode resolver contract, including the resolved `active_yaml_path`, `effective_yaml_path`, runtime `config_path`, interval, and config identity. `paper cycle` now also reads the resolved runtime stage graph and configured ranker directly, so `production`, `parity_baseline`, `stage_debug`, and custom pipeline profiles can switch between persistent execution and preview-only parity workflows without code forks. A read-only `paper status` surface compares the current launch contract against the persisted daemon status JSON to report whether the lane is running, stale, stopped, unhealthy, or restart-required because the launch identity drifted, and the split `paper service` / `paper service apply` surfaces now separate planner from executor: `paper service` stays read-only while `paper service apply` can start, restart, resume, or stop the Rust paper daemon when the lock/status provenance proves Rust ownership. `paper loop` only loads an optional symbols file once at start-up; `paper daemon --watch-symbols-file` can reload that manifest on file changes, keep the last good manifest on invalid or runtime-invalid malformed reloads, continue to union open positions into the active symbol set, start cleanly even before the lane-default watched symbols file exists, and persist a daemon lifecycle status JSON at the resolved `status_path`. The active deployed paper daemon remains `engine/daemon.py` in this phase, but the repo-owned lane launch examples now point at the Rust runner.
...
```
base YAML document
  + promoted config overlay from AI_QUANT_PROMOTED_ROLE (if resolved)
  → _DEFAULT_STRATEGY_CONFIG ← global YAML ← symbols.<SYM> YAML ← live YAML (if live mode) ← modes.<MODE>
```

For Rust paper surfaces, `AI_QUANT_STRATEGY_MODE` is resolved in Rust first;
`AI_QUANT_STRATEGY_MODE_FILE` is the file-backed fallback when the env var is
unset.
