...
## 1. The Trader (Paper)
- **Daemon:** `python -m engine.daemon` (unified loop; paper mode via `AI_QUANT_MODE=paper`)
- **Hot reload:** Strategy YAML (`config/strategy_overrides.yaml`) hot-reloads by mtime via `StrategyManager` (no module reload loop).
...
- **Role:**
  ...
  - Optional per-symbol overrides via `./config/strategy_overrides.yaml`.
...
## 2. The Trader (Live)
- **Daemon:** `python -m engine.daemon` (unified loop; live via `AI_QUANT_MODE=live` or `dry_live`)
...
- **Safety gates:**
  ...
  - Live-only sizing caps in YAML (see `config/strategy_overrides.yaml` → `live:`).
...
- **Powers:**
  ...
  - Adjust strategy parameters via `./config/strategy_overrides.yaml` (hot-reloads automatically).
