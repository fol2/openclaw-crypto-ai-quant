...
- conventional paper lane mapping and launch resolution
...
mkdir -p <lane_project>/artifacts/state && touch <lane_project>/artifacts/state/paper_watchlist_paper1.txt
cargo run -q -p aiq-runtime -- paper lane manifest --lane paper1 --project-dir <lane_project> --symbols ETH --json
cargo run -q -p aiq-runtime -- paper lane apply --lane paper1 --project-dir <lane_project> --symbols ETH --start-step-close-ts-ms 1773424200000 --action auto --json
cargo run -q -p aiq-runtime -- paper lane daemon --lane livepaper --project-dir <lane_project> --symbols ETH --start-step-close-ts-ms 1773424200000 --idle-sleep-ms 1 --max-idle-polls 1 --dry-run --json
...
- `paper lane manifest|apply|daemon --json` must resolve the conventional `paper1` / `paper2` / `paper3` / `livepaper` worktree contracts, inject the documented promoted-role / strategy-mode pair for the candidate lanes only, and keep `livepaper` free of those overlays by convention.
...
- conventional-lane validation should cover at least one candidate lane manifest, one candidate lane apply/start path, and one direct `livepaper` dry-run daemon path.
