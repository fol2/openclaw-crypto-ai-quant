...
The follow-mode smoke reuses the same fixture DB after the earlier bootstrap and
resume `paper loop` commands. On a fresh DB, keep `--start-step-close-ts-ms`
until the first due step has been recorded.
...
- follow-mode validation should prove idle polling does not mutate the DB and that `max_idle_polls` terminates the shell predictably in CI after the configured number of no-work polls.
