...
- `paper service --json` reuses the same read-only status view and reports whether later supervision should hold, start, restart, or merely monitor the lane, together with an operator-facing `action_reason`.
...
- service-action validation should map at least one launch-blocked fixture to `hold`, one launch-ready or stopped fixture to `start`, one stale or mismatched running fixture to `restart`, and one healthy running fixture to `monitor`.
