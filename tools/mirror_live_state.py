...
def mirror_state():
    ...
    try:
        ...
        if args.verbose:
            print(f"Read {len(positions)} positions, balance {live_balance}, {len(orders)} orders, {len(intents)} intents from source.")
        ...
        try:
            ...
            if table_exists(target_conn, "position_state"):
                ...
                if positions:
                    ...
                    target_cur.executemany(f"INSERT INTO position_state ({','.join(pos_columns)}) VALUES ({placeholders})", positions)
            ...
            if table_exists(target_conn, "oms_open_orders"):
                ...
                if orders:
                    ...
                    target_cur.executemany(f"INSERT INTO oms_open_orders ({','.join(order_columns)}) VALUES ({placeholders})", orders)
            ...
            if intents and table_exists(target_conn, "oms_intents"):
                ...
                target_cur.executemany(f"INSERT INTO oms_intents ({','.join(intent_columns)}) VALUES ({placeholders})", intents)

            target_conn.commit()
            
            print(f"Successfully mirrored state:")
